...
// Distributed Tracing (Manual Instrumentation) with OTel-PHP
// This sample demonstrates distributed tracing with context propagation with OpenTelemetry.
...
$tracer = (new TracerProviderFactory('signoz-tracer'))->create()->getTracer();
...
$app->get('/users/{name}', function ($name, Client $client, Response $response) {
    $promises = [
        'service-two'   => $client->getAsync('http://service-two:8000/two/' . $name),
        'service-three' => $client->getAsync('http://service-three:8000/three'),
    ];
